...
ggmap(map) + scalebar(x.min = -122.405, x.max =  -122.365, y.min = 37.705, y.max = 37.72,dist = 2, dist_unit = "km",height=0.1,st.dist = 0.15,transform = TRUE, model = "WGS84")+
  geom_point(daily[[59]], mapping = aes(x=lon,y=lat,size=PM2.5_ATM_ug.m3),color="red")+ scale_size_continuous(limits=c(0,300),breaks = c(0,50,100,150,200,250,300))+
  theme_grey(base_size = 20) + labs(size='Peak Day 09/11')
