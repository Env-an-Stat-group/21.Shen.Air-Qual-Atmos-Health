...
gshhg.l1 = sf::read_sf("C:/Users/Peng Shen/Dropbox/Shen_et_al_2021/R code/gshhg-shp-2.3.7/GSHHS_shp/f/GSHHS_f_L1.shp") %>% st_crop(geo.box)
...
ggplot(data=beforeinterpolation) + geom_raster(aes(x=lon, y=lat,fill=pred)) + 
  scale_fill_gradientn(colours=heat.colors(5),limits=c(floor(rng.before[1]), ceiling(rng.before[2]))) +labs(fill='') + 
...
ggplot(data=afterinterpolation) + geom_raster(aes(x=lon, y=lat,fill=pred)) + 
  scale_fill_gradientn(colours=heat.colors(5),limits=c(floor(rng.after[1]), ceiling(rng.after[2]))) +labs(fill='')+ 
  theme_grey(base_size = 18) + 
  theme(axis.text.x=element_text(size=18),axis.text.y=element_text(size=18))+
  scalebar(x.min = -122.38, x.max = -122.35,y.min = 37.70, y.max = 37.71,location = "bottomright",dist = 2, dist_unit = "km",height=0.2,st.dist = 0.3,transform = TRUE, model = "WGS84")
