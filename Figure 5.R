...
for (k in 36:78){
  ...
  for (i in 1:dim(sf_pop)[1]){
    ...
    mortalitylower[i] = rate*sf_pop$estimate[i]*(exp(0.00052*mean.diff)-1)/exp(0.00052*mean.diff)
    mortalityupper[i] = rate*sf_pop$estimate[i]*(exp(0.00156*mean.diff)-1)/exp(0.00156*mean.diff)
  }
...
ggplot()+geom_sf(data=sf_pop,aes(fill=mortalityrate/estimate*100000)) +
  scale_fill_viridis_c(limits=c(0.5,4),breaks = c(1,2,3,4)) + 
